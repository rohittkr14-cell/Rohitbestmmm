...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    if chat.type not in ['group', 'supergroup'] or not update.message.text:
        return
        ...
        if not is_vouch_forwarded(chat_id, message_id):
            try:
                await context.bot.forward_message(
                    chat_id=VOUCH_FORWARD_CHANNEL_ID,  
                    from_chat_id=chat_id,               
                    message_id=message_id
                )
                mark_vouch_forwarded(chat_id, message_id)
                logger.info(f"✅ Vouch forwarded to channel {VOUCH_FORWARD_CHANNEL_ID}")
            except Exception as e:
    ...
    handler_map = {
        ...
        'unban': cmd_unban, 'mute': cmd_mute, 'unmute': cmd_unmute, 'id': cmd_id,
        'help': cmd_help, 'close': cmd_close, 'start': cmd_start,
        'setinrphoto': cmd_setinrphoto, 'editcrp': cmd_editcrp, 'setvouch': cmd_setvouch,
        'cancel': cmd_cancel,
    }
